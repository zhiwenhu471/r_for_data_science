...
# this is a common setup chunk,means it is run at the start of every chapter
# echo: false means that the code won't be shown in the output
# source("_common.R") runs the code in the _common.R file
```{r}
#| echo: false
source("_common.R")
```
...
Our model of the steps of a typical data science project looks something like @fig-ds-diagram.  # @ means cross-reference
