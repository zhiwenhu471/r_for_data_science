...
`R.options=list(pillar.print_min = 10` means that when printing tibbles, at least 10 rows will be shown before summarizing the output with ellipses. This is useful for seeing more of the data when working with larger datasets.
...
billboard_longer <- billboard |> 
  ...
  mutate(
    week = parse_number(week)  # extract number from week string
  )
billboard_longer
```
...
billboard_longer |> 
  ...
  scale_y_reverse()  # higher ranks are lower numbers, so reverse y-axis
```
