...
flights |>
  ...
  summarize(
    arr_delay = mean(arr_delay, na.rm = TRUE)  # Ignore missing values
  )
```
...
4.  Was there a flight on every day of 2013?
    Display the days that don't have flights.
```{r}
all_days <- tibble(
  month = rep(1:12, times = c(31,28,31,30,31,30,31,31,30,31,30,31)),
  day = unlist(lapply(c(31,28,31,30,31,30,31,31,30,31,30,31), seq_len))
)
```
...
```{r}
# it does matter because `filter()` reduces the number of rows, so if you `filter()` first, `arrange()` has fewer rows to sort, which is more efficient.
...
flights |> 
  mutate(
    gain = dep_delay - arr_delay,
    speed = distance / air_time * 60,
    .before = 1  # Add before the first column
  )
```
...
```{r}
    ggplot(flights, aes(x = dep_time, y = sched_dep_time + dep_delay)) +
...
3.  What happens if you specify the name of the same variable multiple times in a `select()` call?
    ...
    ```
    The variable will only appear once in the output, and it will be in the position of its first occurrence in the `select()` call.
...
4.  What does the `any_of()` function do? 
    ...
    ```
`any_of` selects all the variables that are present in the data frame from the provided vector.
...
7.  Why doesn't the following work, and what does the error mean?
    ...
    flights |> 
      ...
      arrange(arr_delay)
    ```
The error occurs because after using `select(tailnum)`, the resulting data frame only contains the `tailnum` column, and `arr_delay` is no longer present in the data frame to arrange by. To fix this, you need to keep `arr_delay` in the data frame before arranging.
