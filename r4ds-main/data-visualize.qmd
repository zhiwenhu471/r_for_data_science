...
1.  How many rows are in `penguins`? `r nrow(penguins)`
    How many columns? `r ncol(penguins)`
...
#| warning: false
#| fig-alt: |
#|   A scatterplot of bill depth vs. bill length of penguins. There is no
...
#| warning: false
#| fig-alt: |
#|  A scatterplot of bill depth vs. species of penguins. The plot displays
#|  three vertical columns of points, one for each species (Adelie,
#|  Chinstrap, and Gentoo). Gentoo penguins tend to have the deepest
#|  bills, followed by Chinstrap and then Adelie penguins.
#|  There is considerable overlap in bill depths
#|  between the three species.
#|  The distribution of bill depths appears to be
#|  right skewed for each species.
...
6.  What does the `na.rm` argument do in `geom_point()`?
    `na.rm` is short for "NA remove".It controls whether missing values (NAs) are removed before plotting.
...
9.  Run this code in your head and predict what the output will look like.
    ...
    ggplot(
      ...
      geom_point() +
      geom_smooth(se = FALSE)  # 'se = FALSE' turns off the confidence interval shading
    ```
...
1.  Run the following lines of code.
    ...
    the latter is the one saved.
    
    Why?
    because ggsave() saves the most recently created plot. The second ggplot() call creates a new plot that overwrites the first one.
